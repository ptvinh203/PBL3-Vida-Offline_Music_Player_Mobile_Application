...
public class TrieServiceImpl implements TrieService{
    ...
    public Trie buildArtistTrie() throws IOException, ClassNotFoundException{
        ...
        ObjectInputStream objectInputStream = new ObjectInputStream(new FileInputStream(Paths.get(ARTIST_FILE_URL).toFile()));
    ...
    public Trie buildSongTrie() throws IOException, ClassNotFoundException{
        Trie result = new Trie();
        ObjectInputStream objectInputStream = new ObjectInputStream(new FileInputStream(Paths.get(SONG_FILE_URL).toFile()));
        result.setRoot((TrieNode)objectInputStream.readObject());
        objectInputStream.close();
        return result;
    }
    ...
    @Override
    public List<String> search(String prefix, boolean isArtist) {
        if (isArtist) {
    ...
    public void saveToFile(boolean isArtist) throws IOException{
        if (isArtist) {
            ObjectOutputStream objectOutputStream = new ObjectOutputStream(new FileOutputStream(Paths.get(ARTIST_FILE_URL).toFile()));
            objectOutputStream.writeObject(artistTrie.getRoot());
            objectOutputStream.close();
        }
        else {
            ObjectOutputStream objectOutputStream = new ObjectOutputStream(new FileOutputStream(Paths.get(SONG_FILE_URL).toFile()));
    ...
    public void insert(String newName, boolean isArtist) throws IOException {
        if (isArtist) artistTrie.insert(newName);
        else songTrie.insert(newName);
    ...
    public void delete(String name, boolean isArtist) throws IOException {
        if (isArtist) artistTrie.delete(name);        
        else songTrie.delete(name);   
